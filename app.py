...
<<<<<<< HEAD
    ...
    if answer.lower() == "reset":
        conversation_state = { "severity_node" : 0, "asked_symptoms" : []}
        return jsonify({"response" : "The conversation state is reset"}) , 200
    ints = predict_class(answer,model) 
    severity_node = conversation_state["severity_node"]
    ...
    if ints[0]["intent"] == "Positive":
        ...
        asked_symptoms = conversation_state["asked_symptoms"]
        if len(asked_symptoms) == 0:
            conversation_state["asked_symptoms"].append(feature_name)
        ...
        conversation_state["severity_node"] = severity_node
